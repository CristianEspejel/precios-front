...
import { getAllProduct } from '../services/productMServices';
import TableVenta from '../components/tableVenta';
import Pagination from '../components/Pagination/Pagination';
import CobroConTarjeta from '../components/CobroTrajeta';
import addIcon from '../assets/add.png';
import minusIcon from '../assets/menos.png';

const RealizarVentaM = () => {
  ...
  const handleAddToVenta = (product, cantidad) => {
    const existingProduct = productosSeleccionados.find(p => p.id === product.id);
    const productPrice = product.price * cantidad;
    if (existingProduct) {
      ...
      setProductosSeleccionados([...productosSeleccionados, { ...product, cantidad }]);
    }
    setTotalVenta(totalVenta + productPrice);
  };
    ...
    <section className="bg-gray-50 dark:bg-gray-900 min-h-screen py-12 sm:ml-26 mt-12">
      <div className="mx-auto px-4 lg:px-12">
        <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden p-6">
          ...
          <CobroConTarjeta totalVenta={totalVenta} />
        </div>
      </div>
    </section>
  );
